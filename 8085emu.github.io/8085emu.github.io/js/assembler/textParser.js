    ...
    for (var i in lines) {
                ...
                for (var element in buffer) {
                    for (var pointer = buffer[element].length - 2; pointer >= 0; pointer -= 2) {
                        ...
                        listing[i].code.push({
                            ...
                            code: buffer[element].slice(pointer, pointer + 2)
                        });
        ...
        if (!errFlag) {
            try {
                switch (args.length) {
                    case 0:
                        instrCode = assembler.codeOf[instr].code;
                        break;
                    case 1:
                        instrCode = assembler.codeOf[instr][accessor[0]].code;
                        break;
                    case 2:
                        instrCode = assembler.codeOf[instr][accessor[0]][accessor[1]].code;
                        break;
                    default:
                        errFlag = true;
                        break;
            ...
            if (types[j] == '8-bit') {
                ...
                if (references.hasOwnProperty(args[j])) {
                    ...
                    if (forwardReferences.hasOwnProperty(args[j])) {
                        ...
                        //Necessary for listing
                        forwardReferencesListing[args[j]] = {
                            at: [],
                            foundAt: false
                        };
                        forwardReferencesListing[args[j]].at.push(i);
    ...
    for (var label in forwardReferences) {
        for (var line in forwardReferences[label]) {
            ...
            if (references.hasOwnProperty(label)) {
                ...
                listing[forwardReferencesLines[currentLine]].code.push({
                    location: currentLine,
                    code: arg.slice(2)
                });
                listing[forwardReferencesLines[currentLine]].code.push({
                    location: assembler.helpers.incrementHex(currentLine),
                    code: arg.slice(0, 2)
                });
