...
registers.getFlag = function(flag) {
    ...
    switch (flag) {
        case 'S':
            val = conversion.hexToBin(registers.F, 8)[0];
            break;
        case 'Z':
            val = conversion.hexToBin(registers.F, 8)[1];
            break;
        case 'AC':
            val = conversion.hexToBin(registers.F, 8)[3];
            break;
        case 'P':
            val = conversion.hexToBin(registers.F, 8)[5];
            break;
        case 'CY':
            val = conversion.hexToBin(registers.F, 8)[7];
            break;
        default:
            val = false;
            break;
...
registers.setFlag = function(flag, onOff) {
    switch (flag) {
        case 'S':
            registers.F = registers._mask(registers.F, 0, onOff);
            break;
        case 'Z':
            registers.F = registers._mask(registers.F, 1, onOff);
            break;
        case 'AC':
            registers.F = registers._mask(registers.F, 3, onOff);
            break;
        case 'P':
            registers.F = registers._mask(registers.F, 5, onOff);
            break;
        case 'CY':
            registers.F = registers._mask(registers.F, 7, onOff);
            break;
        default:
            registers.F = false;
            break;
...
registers.getPair = function(pair) {
    ...
    switch (pair) {
        case 'PSW':
            val += registers.A + registers.F;
            break;
        case 'B':
            val += registers.B + registers.C;
            break;
        case 'D':
            val += registers.D + registers.E;
            break;
        case 'H':
            val += registers.H + registers.L;
            break;
        case 'SP':
            val += registers.SP;
            break;
...
registers.setPair = function(pair, value) {
    ...
    switch (pair) {
        ...
        case 'WZ':
            ...
            break;
        case 'SP':
            registers.SP = higher + lower;
            break;
        case 'PC':
            registers.PC = higher + lower;
            break;
    }
    pbus.dispatchEvent(pbus.events.updatedRegister);
    return value;
